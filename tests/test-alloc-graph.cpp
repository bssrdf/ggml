...
void load_model(test_model & model, float* a, float* b, int M, int N, int K, int L, bool use_gpu = false) {
    ...
    ggml_tallocr_alloc(&alloc, model.encode1_bias);
    ggml_tallocr_alloc(&alloc, model.encode1_weight);
...
struct ggml_cgraph * compute(const test_model & model, ggml_gallocr_t allocr) {
...
int main(void)
{
    ...
    for(int i = 0; i < ggml_graph_n_nodes(gf); ++i) {
        if(strcmp(ggml_get_name(ggml_graph_node(gf, i)), "result") == 0) {
            result = ggml_graph_node(gf, i);
        } else if(strcmp(ggml_get_name(ggml_graph_node(gf, i)), "encode1_w") == 0) {
            encode1_w = ggml_graph_node(gf, i);
        }
    }
