...
void load_model(test_model & model, float* w, float* b, int M, int N, int K, bool use_gpu = false) {
    ...
    model.w = ggml_new_tensor_2d(model.ctx, GGML_TYPE_F32, M, N);
    ...
    ggml_set_param(model.ctx, model.w);
    ggml_set_param(model.ctx, model.b);
...
struct ggml_cgraph * build_graph(const test_model& model, struct ggml_allocr * allocr) {
    ...
    // zT = x @ yT
    struct ggml_tensor * wx = ggml_mul_mat(ctx0, model.w, model.input);
    struct ggml_tensor * result = ggml_add(ctx0, wx, ggml_repeat(ctx0, model.b, wx));
...
struct ggml_tensor* compute(const test_model & model, float *input, struct ggml_allocr * allocr) {
    ...
    struct ggml_tensor * in = get_tensor_from_graph(gf, "input");
    GGML_ASSERT(in != NULL);
    ...
    if(ggml_backend_is_cpu(model.backend)
        ...
        memcpy(in->data, input, ggml_nbytes(in));
    } else {
        ggml_backend_tensor_set(in, input, 0, ggml_nbytes(in)); // cuda requires copy the data directly to device
    }
...
int main(void)
{
    ...
    // matrix A (4 X 36)
    float matrixA[M*N] = {1.0f, 2.0f, 3.0f, 
                          4.0f, 5.0f, 6.0f,
                          7.0f, 8.0f, 9.0f, 
                          10.0f, 11.0f, 12.0f  };
   ...
   float expected_result[K * N] = {15.f, 34.f, 53.f, 72.0, 
    ...
    struct ggml_tensor * result = compute(model, input, allocr);
