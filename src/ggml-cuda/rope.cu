...
static __global__ void rope_multi(
        const T * x, T * dst, const int ne0, const int ne1, const int ne2, const int s01, const int s02,
        ...
        const bool is_imrope, const int s1, const int s2, const bool permute) {
    const int i0 = 2*(blockDim.y*blockIdx.y + threadIdx.y);
    ...
    if (is_imrope) {
        ...
        ix   = channel_x*s02 + row_x*s01 + i0;
    } else {
        idst = row_dst*ne0 + i0/2;
        ix   = channel_x*s02 + row_x*s01 + i0/2;
    }
    ...
    if (is_imrope) {
        ...
        dst[idst + 0]  = ggml_cuda_cast<T>(x0*cos_theta - x1*sin_theta);
        dst[idst + 1]  = ggml_cuda_cast<T>(x0*sin_theta + x1*cos_theta);
    } else {
        const float x0 = ggml_cuda_cast<float>(x[ix + 0]);
        const float x1 = ggml_cuda_cast<float>(x[ix + n_dims/2]);

        dst[idst + 0]        = ggml_cuda_cast<T>(x0*cos_theta - x1*sin_theta);
        dst[idst + n_dims/2] = ggml_cuda_cast<T>(x0*sin_theta + x1*cos_theta);
    }
}
...
static void rope_multi_cuda(
        const T * x, T * dst, const int ne0, const int ne1, const int ne2, const int s01, const int s02, const int n_dims, const int nr,
        const int32_t * pos, const float freq_scale, const float freq_base, const float ext_factor, const float attn_factor,
        const rope_corr_dims corr_dims, const float * freq_factors, const mrope_sections sections, const bool is_imrope,
        const int s1, const int s2, const bool permute, cudaStream_t stream) {
    GGML_ASSERT(ne0 % 2 == 0);
    const dim3 block_dims(1, CUDA_ROPE_BLOCK_SIZE, 1);
    const int n_blocks_x = (ne0 + 2*CUDA_ROPE_BLOCK_SIZE - 1) / (2*CUDA_ROPE_BLOCK_SIZE);
    const dim3 block_nums(nr, n_blocks_x, 1);
...
void ggml_cuda_op_rope_impl(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (is_neox) {
        if (src0->type == GGML_TYPE_F32) {
            ...
            rope_neox_cuda<forward>(
                (const nv_bfloat16 *) src0_d, (nv_bfloat16 *) dst_d, ne00, ne01, s01, s02, n_dims, nr, pos, freq_scale,
                freq_base, ext_factor, attn_factor, corr_dims, freq_factors, stream);
        } else {
        ...
        if (src0->type == GGML_TYPE_F32) {
            rope_multi_cuda<forward>(
                ...
                freq_base, ext_factor, attn_factor, corr_dims, freq_factors, sections, is_imrope, s1, s2, permute_layout, stream);
        } else if (src0->type == GGML_TYPE_F16) {
            rope_multi_cuda<forward>(
            ...
            rope_multi_cuda<forward>(
                (const nv_bfloat16 *) src0_d, (nv_bfloat16 *) dst_d, ne00, ne01, ne02, s01, s02, n_dims, nr, pos, freq_scale,
                freq_base, ext_factor, attn_factor, corr_dims, freq_factors, sections, is_imrope, s1, s2, permute_layout, stream);
        } else {
            GGML_ABORT("fatal error");
        }
